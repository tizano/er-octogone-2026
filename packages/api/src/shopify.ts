...
function shopDomain(): string {
  return env.SHOPIFY_SHOP.includes('.')
    ? env.SHOPIFY_SHOP
    : `${env.SHOPIFY_SHOP}.myshopify.com`;
}
...
export async function createDraftOrder(
  ...
  const data = await graphql<GqlDraftOrderCreateResponse>(
...
export async function completeDraftOrder(
  ...
  const data = await graphql<GqlDraftOrderCompleteResponse>(
